...
- ✅ **M8 8a 완료** — 빌드 다양성 토대+런타임: `WeaponInstance`/`BuildState`/`UpgradePool`, `WeaponController` 인벤토리(Nearest/Spread/Aura 발사), 동적 업그레이드 풀(무기/패시브/진화 혼합), 콘텐츠(무기 4종·패시브 6종·진화 1종). 테스트 84/84, 플레이 실측(다무기 발사·동적 카드).
- 🎯 **다음 = M8 8b** — Lance(관통)·Orbit(궤도) fireMode + 나머지 진화 4종 + (선택) 오라/진화 VFX. 스펙은 위 "마일스톤 8" 참조.
- ⏳ 이후 후보: 난도 곡선+보스/엘리트(Leaf), 손맛/UX 폴리시(카드 아이콘·데미지 숫자·SFX), 메타 progression+캐릭터 선택
